...
.. .1  //输出3331
      ...
      points: [
        ...
        '@include的运作方式其实就是在@include语句的位置直接粘贴库的代码，和C语言的include类似。因此其事实上并不一定局限于def语句',
...
block main {
  example (11, 11|11)
      ...
      points: [
        ...
        '宏的参数名规范：建议使用name_l或name_b来标明这个参数在传入时应当传入大端还是小端。l：小端，b:大端',
        '宏命名（尤其是基础gadgets）建议遵循以下规范：如果执行后伴随POP效果，则在尾部标明',
        '例如执行后会pop xr4qr8，则在名字的尾部标上"_X4Q8"',
        '如果这个宏是一个gadget，且没有将寄存器传参包装为参数，则在头上加上"__"(双下划线)',
        '如果执行时还伴随其它“一言难尽”的副作用，则直接在最尾部加上"__"(双下划线)',
        '如果宏的名字以$开头，则可以在宏的名字中写特殊符号',
        '如果要输入以$开头的宏并触发自动补全，不要在开头直接输入$。由于技术原因。以$开头似乎难以参与自动补全匹配。欢迎发补丁'
      ]
    },
    {
      id: 'labels 地址标签',
...
block main {
  example(arg:2b)  //这样就可以直接使用这个函数了
      ...
      points: [
        ...
        '库命名仅支持：以字母或下划线开头，后面跟若干字母数字下划线，不建议起奇形怪状的名字'
      ]
    },
    {
      id: 'yield 宏-yield字段',
