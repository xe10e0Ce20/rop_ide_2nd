  ...
  const filteredLibs = useMemo(() => {
    ...
    if (term) {
      result = result.filter(lib =>
        ...
        lib.author.toLowerCase().includes(term)
      );
    }
    result.sort((a, b) => {
      const aStar = (a as any).starred ? 1 : 0;
      const bStar = (b as any).starred ? 1 : 0;
      ...
      <div style={{ width: '920px', height: '620px', background: '#161616', border: '1px solid #2d2d2d', borderRadius: '8px', display: 'flex', flexDirection: 'column', fontFamily: "'JetBrains Mono', monospace", color: '#e0e0e0' }}>
        ...
        <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
          ...
          <div style={{ flex: 1, padding: '20px', display: 'flex', flexDirection: 'column', gap: '12px', overflowY: 'auto' }}>
            {/* 搜索框 + 刷新按钮 */}
            <div style={{ display: 'flex', gap: '8px', alignItems: 'center', borderBottom: '1px solid #222', paddingBottom: '6px' }}>
              <input
                type="text"
                placeholder="🔍 搜索名称或作者..."
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                style={{ flex: 1, background: '#111', border: '1px solid #333', color: '#fff', padding: '6px 10px', fontSize: '12px', borderRadius: '4px', outline: 'none' }}
              />
              {onManualRefresh && (
                <button type="button" onClick={onManualRefresh} disabled={isRefreshing} style={{ background: '#222', border: '1px solid #333', color: isRefreshing ? '#555' : '#00ffb3', padding: '6px 12px', fontSize: '11px', borderRadius: '4px', cursor: isRefreshing ? 'not-allowed' : 'pointer', whiteSpace: 'nowrap' }}>
            ...
            <div style={{ fontSize: '12px', color: '#888', marginTop: '4px' }}>
              当前视图：{assetType === 'package' ? `依赖库 (${filteredLibs.length})` : `代码片段 (${filteredSnippets.length})`}
            </div>

            {/* 依赖库列表 */}
            {assetType === 'package' && filteredLibs.map(lib => {
              const starred = (lib as any).starred === true;
              return (
                <div key={`${lib.name}@${lib.isLocal ? 'local' : 'cloud'}`} style={{ background: '#111', border: `1px solid ${starred ? '#fbbf24' : '#222'}`, borderRadius: '6px', padding: '12px', display: 'flex', flexDirection: 'column', gap: '8px', boxShadow: starred ? '0 0 8px rgba(251,191,36,0.3)' : 'none' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                      {starred && <span style={{ color: '#fbbf24', fontSize: '16px', marginRight: '4px', verticalAlign: 'middle' }}>⭐</span>}
            ...
            {/* 代码片段列表 */}
            {assetType === 'snippet' && filteredSnippets.map(snippet => {
              const starred = (snippet as any).starred === true;
              return (
                <div key={`${snippet.title}@${snippet.isLocal ? 'local' : 'cloud'}`} style={{ background: '#111', border: `1px solid ${starred ? '#fbbf24' : '#222'}`, borderRadius: '6px', padding: '12px', display: 'flex', flexDirection: 'column', gap: '8px', boxShadow: starred ? '0 0 8px rgba(251,191,36,0.3)' : 'none' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                      {starred && <span style={{ color: '#fbbf24', fontSize: '16px', marginRight: '4px' }}>⭐</span>}
